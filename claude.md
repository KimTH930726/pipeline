...
- **Deploy 검증**: 승인 여부 + 브랜치 존재 + 인증 필수 (API 레벨), 배포 성공 시 리뷰 PENDING 초기화 + 샌드박스 자동 삭제
- **Deploy Docker**: backend=`restart`(볼륨 마운트), frontend=`build --no-cache` + `up --no-deps`(compose -p 프로젝트명 지정)
...
Deploy:   POST /api/deploy/  GET /api/deploy/recent?status=&branch=|status/{id}|compare/{a}/{b}
          WS /api/deploy/ws/{id}

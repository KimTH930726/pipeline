...
class GetRecentDeployments:
    ...
    async def execute(self, branch: str | None = None, status: str | None = None, date_from: str | None = None, date_to: str | None = None, page: int = 1, size: int = 10) -> tuple[list[DeployStatusDTO], int]:
        offset = (page - 1) * size
        total = await self._repo.count(branch, status, date_from, date_to)
        deps = await self._repo.find_recent(branch, status=status, date_from=date_from, date_to=date_to, limit=size, offset=offset)
        items = [
            DeployStatusDTO(
